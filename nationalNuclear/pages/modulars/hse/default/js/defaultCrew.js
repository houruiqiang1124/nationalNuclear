...
new Vue({
    ...
    mounted: function() {
        _this = this;
        function plusReady(){
            _this.checkedUser = JSON.parse(localStorage.getItem("defaultCrew")) || {};
            _this.getCopyPerson();
        }
        if (window.plus) {
            plusReady()
    ...
    methods: {
        // 确定
        ok: function() {
            localStorage.setItem("defaultCrew",JSON.stringify(_this.checkedUser))
            mui.back();
        },
        // 选中人员
        	...
        	app.ajax({
        		...
        		success: function(res) {
                    ...
                    for(var i = 0; i < res.beans.length; i++) {
                        if(defaultCrew) {
                            if(res.beans[i].unitId == defaultCrew.unitId) {
